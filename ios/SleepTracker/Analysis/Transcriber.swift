...
final class Transcriber {
    ...
    var unavailableReason: String? {
        guard let recognizer else { return "no recogniser for \(Locale.current.identifier)" }
        ...
        if !recognizer.supportsOnDeviceRecognition {
            return "no on-device model for \(recognizer.locale.identifier)"
        }
        if !recognizer.isAvailable {
            return "recogniser temporarily unavailable"
        }
        return nil
    }
